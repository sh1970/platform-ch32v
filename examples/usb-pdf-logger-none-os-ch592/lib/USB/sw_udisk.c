...
void UDISK_CMD_Deal_Fail( void )
{
    ...
    {
        /* EP3 -> STALL */
        R8_UEP3_CTRL = ( R8_UEP3_CTRL & ~MASK_UEP_R_RES ) | UEP_R_RES_STALL;
...
void UDISK_Up_OnePack( void )
{
    ...
    FLASH_RD_Block( RAM_BUFFER.PDF_BUFFER.UDisk_Pack_Buffer, UDISK_Pack_Size );
    pbuf = RAM_BUFFER.PDF_BUFFER.UDisk_Pack_Buffer;
...
//#if (STORAGE_MEDIUM == MEDIUM_SPI_FLASH)
//        PRINT("sec_start_addr:%x\n", sec_start_addr);
//        FLASH_RD_Block_Start( sec_start_addr );
//        FLASH_RD_Block(TempBuffer, 4096);
//        FLASH_RD_Block_End();
//        FLASH_Erase_Sector(sec_start_addr);
//        W25XXX_WR_Block(&TempBuffer[DEF_FLASH_SECTOR_SIZE], sec_start_addr+DEF_FLASH_SECTOR_SIZE, EEPROM_BLOCK_SIZE-DEF_FLASH_SECTOR_SIZE );
//#endif
    }
    pdf_memcpy(RAM_BUFFER.PDF_BUFFER.UDisk_Down_Buffer + (uint32_t)UDISK_Sec_Pack_Count * UDISK_Pack_Size ,pbuf, UDISK_Pack_Size);
    UDISK_Sec_Pack_Count++;
    UDISK_Transfer_DataLen -= UDISK_Pack_Size;
...
#if (STORAGE_MEDIUM == MEDIUM_SPI_FLASH)
        W25XXX_WR_512B(RAM_BUFFER.PDF_BUFFER.UDisk_Down_Buffer, sec_start_addr, DEF_FLASH_SECTOR_SIZE );
#elif (STORAGE_MEDIUM == MEDIUM_INTERAL_FLASH)
        IFlash_Prog_512(IFLASH_UDISK_START_ADDR + sec_start_addr,(uint32_t*)RAM_BUFFER.PDF_BUFFER.UDisk_Down_Buffer);
#endif
        if( UDISK_Transfer_DataLen == 0x00 )
        {
            Udisk_Transfer_Status &= ~DEF_UDISK_BLUCK_DOWN_FLAG;
